...
const EditBook = () => {
  ...
  const fetchBook = async () => {
    try {
      ...
      if (!genres.filter(g => g !== 'Other').includes(book.genre)) {
        setCustomGenre(book.genre);
        setFormData(prev => ({ ...prev, genre: "Other" }));
      }
    } catch (error) {
      console.error(error);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate("/books", {
        state: { successMsg: `"${formData.title}" updated successfully.` },
      });
    } catch (err) {
        ...
        <Card>
          ...
          <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground">Leave empty to keep current cover image.</p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground">Leave empty to keep current ebook.</p>
        ...
        <div className="flex justify-end gap-3 pt-2">
          ...
          >
            {loading ? (
              ...
              <><Save className="mr-2 h-4 w-4" /> Update Book</>
            )}
          </Button>
        </div>
      </form>
