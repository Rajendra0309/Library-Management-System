...
const MemberProfile = () => {
  ...
  const [activeTab, setActiveTab] = useState('active'); // active, history, reservations, fines
      ...
      <section className="bg-surface rounded-xl p-card-padding shadow-sm border border-border-subtle mb-4xl">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            ...
            <div className="flex-1">
              {editMode ? (
                <div className="space-y-3 max-w-md">
                  <input type="text" name="name" value={updatedMember.name} onChange={handleEditChange} className="w-full font-headline-2xl text-headline-2xl text-on-surface mb-1 bg-surface-bright border border-border-default rounded px-2 py-1 focus:ring-2 focus:ring-primary focus:outline-none" />
                  <input type="text" name="phone" placeholder="Phone" value={updatedMember.phone} onChange={handleEditChange} className="w-full font-body-sm text-body-sm text-on-surface mb-1 bg-surface-bright border border-border-default rounded px-2 py-1 focus:ring-2 focus:ring-primary focus:outline-none" />
                  <input type="text" name="profileImage" placeholder="Profile Image URL" value={updatedMember.profileImage} onChange={handleEditChange} className="w-full font-body-sm text-body-sm text-on-surface mb-1 bg-surface-bright border border-border-default rounded px-2 py-1 focus:ring-2 focus:ring-primary focus:outline-none" />
                </div>
              ) : (
                <>
          ...
          <div className="flex items-center flex-wrap gap-3 w-full md:w-auto mt-4 md:mt-0">
            {editMode ? (
               <>
                 <button onClick={handleSaveProfile} disabled={updating} className="flex-1 md:flex-none px-4 py-2 bg-primary text-on-primary rounded-lg font-body-sm text-body-sm hover:shadow-brand-glow hover:-translate-y-[2px] transition-all flex items-center gap-2 justify-center">
                   <span className="material-symbols-outlined text-sm">save</span> Save
                 </button>
                 <button onClick={() => setEditMode(false)} className="flex-1 md:flex-none px-4 py-2 bg-surface border border-border-default text-text-secondary rounded-lg font-body-sm text-body-sm hover:bg-bg-hover transition-all">
                   Cancel
                 </button>
               </>
            ) : (
              (isSelf || isLibrarianOrAdmin) && (
                <button onClick={() => setEditMode(true)} className="flex-1 md:flex-none px-4 py-2 bg-primary text-on-primary rounded-lg font-body-sm text-body-sm hover:shadow-brand-glow hover:-translate-y-[2px] transition-all flex items-center gap-2 justify-center">
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-xl mb-4xl">
          {activeBorrows.length === 0 ? (
            ...
            activeBorrows.map(borrow => (
              <div key={borrow._id} className="bg-surface rounded-xl p-card-padding shadow-sm border border-border-subtle hover:shadow-md hover:-translate-y-[3px] transition-all group">
                <div className="flex gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="font-headline-lg text-headline-lg text-on-surface line-clamp-2 leading-tight mb-2" title={borrow.bookId?.title}>
             ...
             <table className="w-full text-left border-collapse">
                ...
                <tbody className="font-body-sm text-body-sm divide-y divide-border-subtle">
                  {borrowHistory.length === 0 ? (
                     <tr>
                        <td colSpan="4" className="py-8 text-center text-text-secondary">No historical borrowing records.</td>
                     </tr>
                  ) : (
                     borrowHistory.map(historyItem => (
                       <tr key={historyItem._id} className="hover:bg-bg-hover transition-colors bg-bg-surface">
                         <td className="py-3 px-card-padding font-medium text-on-surface">{historyItem.bookId?.title || 'Unknown Title'}</td>
